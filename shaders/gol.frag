...
void main() {
	gl_FragColor = vec4(texture2D(tex, vec2(gl_FragCoord.x, gl_FragCoord.y)).r, texture2D(tex, vec2(gl_FragCoord.x, gl_FragCoord.y)).g, texture2D(tex, vec2(gl_FragCoord.x, gl_FragCoord.y)).b, 1.0);
}